...
class RoomNoteLocalDataSource @Inject constructor(
    ...
    override suspend fun addNote(note: Note) = withContext(dispatcherIO) {
        noteDao.upsertNote(NoteEntity.fromNote(note))
    }

    override suspend fun updateNote(updatedNote: Note) = withContext(dispatcherIO) {
        noteDao.upsertNote(NoteEntity.fromNote(updatedNote))
    }

    override suspend fun removeNote(note: Note) = withContext(dispatcherIO) {
        noteDao.deleteNoteById(note.id)
    }

    override suspend fun deleteAllNotes() = withContext(dispatcherIO) {
        noteDao.deleteAllNotes()
    }

    override suspend fun getNoteById(noteId: String): Note? = withContext(dispatcherIO) {
